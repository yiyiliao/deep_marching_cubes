...
import numpy as np
#import settings
from utils.util import offset_to_normal, offset_to_vertices, pts_in_cell, dis_to_meshs
from model.table import get_connected_pairs, get_accept_topology
...
class LossAutoGrad(object):
    ...
    def loss_point_to_mesh_distance_autograd(self, offset, point, phase='train'):
        ...
        for i_,(x_,y_,z_) in enumerate(zip(self.xv_value, self.yv_value, self.zv_value)): 
            ...
            pts_cell = pts_in_cell(torch.unsqueeze(point, 0), [x_,y_,z_,
                x_+self.args.len_cell,y_+self.args.len_cell,z_+self.args.len_cell])
            if len(pts_cell)==0:
                dis = dis_empty
    ...
    def loss_on_curvature_autograd(self, offset, topology):
		...
		# similarity constraint matrix
                ...
                p1 = Variable((F.softmax(topology[i_,:], dim=0).data).type(self.dtype))
                p2 = Variable((F.softmax(topology[i_+self.args.num_cells*self.args.num_cells,:], dim=0).data).type(self.dtype))
		...
		# similarity constraint matrix
                ...
                # as the topology is only taken as a constant weight matrix 
                p1 = Variable((F.softmax(topology[i_,:], dim=0).data).type(self.dtype))
                p2 = Variable((F.softmax(topology[i_+self.args.num_cells,:], dim=0).data).type(self.dtype))
		...
		# similarity constraint matrix
                ...
                # as the topology is only taken as a constant weight matrix 
                p1 = Variable((F.softmax(topology[i_,:], dim=0).data).type(self.dtype))
                p2 = Variable((F.softmax(topology[i_+1,:], dim=0).data).type(self.dtype))
